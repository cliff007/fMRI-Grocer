function [varargout]=FG_rest_sliceviewer(AOperation, varargin)
% Show a brain's slice. "FG_rest_sliceviewer" can be opened more than one instance like MRIcro, and supports multi-slice, overlay and so on. by Xiao-Wei Song
%Usage: hFig =FG_rest_sliceviewer('ShowImage', AFilename, CallBack);
%           FG_rest_sliceviewer('Delete', AFigHandle);
%Detailed usage is the code file "FG_rest_sliceviewer.m"
%------------------------------------------------------------------------------------------------------------------------------
...
%   Last Modified by DONG Zhang-Ye and YAN Chao-Gan 101025: Fixed a bug in displaying with MATLAB 2010; support mouse wheel operation.
%   Last Modified by DONG Zhang-Ye and YAN Chao-Gan 110505: Fixed a bug in dispalying "only -"; Fixed a bug in dispaying unerlay with 2x2x2 voxel size.

if nargin<1, %No param Launch, 20070918
	%help(mfilename);
	if nargout>=1,
		varargout{1}=FG_rest_sliceviewer('ShowImage','');	%by Default, I show a black brain image
	else
		FG_rest_sliceviewer('ShowImage','');	%by Default, I show a black brain image
	end
	return; 
end
...
%try
	...
	case 'SHOWIMAGE',	%ShowImage
		if nargin==2,
			%FG_rest_sliceviewer('ShowImage', theBrainMap); %reho_gui.m 989
			AFilename =varargin{1};
			ACallback ='';		
	    elseif nargin==3,
			%FG_rest_sliceviewer('ShowImage', theBrainMap, [theCallback cmdClearVar], 'Power Spectrum');%reho_gui.m 1010
			AFilename =varargin{1};
			ACallback =varargin{2};		
		else
			error(sprintf('Usage: hFig =FG_rest_sliceviewer(''ShowImage'', AFilename); \n\t hFig =FG_rest_sliceviewer(''ShowImage'', AFilename, ACallback);')); 
		end
		...
		% isExistFig =FG_rest_misc( 'ForceCheckExistFigure' , theFig);	%Force check whether the figure exist
		% if ~isExistFig
	...
	case 'UPDATECALLBACK', 		%UpdateCallback
		...
		elseif nargin==4, 
			...
			error('Usage: FG_rest_sliceviewer(''UpdateCallback'', AFigureHandle, ACallback, ACallbackCaption);'); 
		end
	...
	case 'DELETE', % Delete specific figure
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''Delete'', AFigHandle);'); end
		AFigHandle =varargin{1};
		REST_SliceViewer_Cfg =DeleteFigure(REST_SliceViewer_Cfg, AFigHandle);
		
		if ~GetDisplayCount(REST_SliceViewer_Cfg),
			FG_rest_sliceviewer('QuitAllSliceViewer');
		end
		
	case {'CLICKPOSITION', 'SETPOSITION'},		%ClickPosition	%SetPosition
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''ClickPosition'', AFigHandle); or FG_rest_sliceviewer(''SetPosition'', AFigHandle);'); end
		AFigHandle =varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
			...
			if isSelfYoked ,
				...
				for x=1:GetDisplayCount(REST_SliceViewer_Cfg), 
					if x~=theCardinal ,
						...
						if isYoked, 							
							FG_rest_sliceviewer('SetPhysicalPosition', REST_SliceViewer_Cfg.Config(x).hFig, theDistanceFromOrigin);
						end
					end
				end
			end	
			...
			FG_rest_sliceviewer('RunCallback', AFigHandle);			
		end
		
	case 'SETPHYSICALPOSITION',		%SetPhysicalPosition, for Yoke, the physical position(mm) from the origin
		if nargin~=3, error('Usage: FG_rest_sliceviewer(''SetPhysicalPosition'', AFigHandle, ADistanceFromOrigin);'); end
		AFigHandle =varargin{1};
	...
	case 'RUNCALLBACK', 			%RunCallback	, 20070625
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''RunCallback'', AFigHandle);'); end
		AFigHandle =varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0		
			% Run the Callback
	...
	case 'GETPOSITION', 		%GetPosition, return Absolute	position where current cross-hair stay
		if nargin~=2, error('Usage: Position =FG_rest_sliceviewer(''GetPosition'', AFigHandle);'); end
		AFigHandle =varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
	...
	case 'GETPHYSICALPOSITION', 		%GetPhysicalPosition return Coordinate .* VoxelSize	(mm)
		if nargin~=2, error('Usage: DistanceFromOrigin =FG_rest_sliceviewer(''GetPhysicalPosition'', AFigHandle);'); end
		AFigHandle =varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
	...
	case 'SETMESSAGE',			%SetMessage, update the Message, I don't contain the Copyright any more , 20070915
		if nargin~=3, error('Usage: FG_rest_sliceviewer(''SetMessage'', AFigHandle, AMessage);'); end
		AFigHandle 	=varargin{1};
		AMessage	=varargin{2};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
	...
	case 'MAGNIFY', 		%Magnify, x0.5 -x1 -x2 -x3 -x4 -x5 -x6
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''Magnify'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
	...
	case 'QUITALLSLICEVIEWER',		%QuitAllSliceViewer
		if nargin~=1, error('Usage: FG_rest_sliceviewer(''QuitAllSliceViewer'');'); end
		for x=GetDisplayCount(REST_SliceViewer_Cfg):-1:1, % DawnSong, revised 20070625
			FG_rest_sliceviewer('DELETE', REST_SliceViewer_Cfg.Config(x).Filename);
		end
		if mislocked(mfilename),munlock; end
		clear REST_SliceViewer_Cfg;
		
	case 'MONTAGE', 		%Montage	
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''Montage'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'ORTHOGONALVIEW', 		%OrthogonalView
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''OrthogonalView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'TRANSVERSEVIEW', 		%TransverseView
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''TransverseView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'SAGITTALVIEW', 		%SagittalView
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''SagittalView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'CORONALVIEW', 		%CoronalView
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''CoronalView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'REPAINT',		%Repaint	
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''Repaint'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,		
			REST_SliceViewer_Cfg.Config(theCardinal) =SetImage(REST_SliceViewer_Cfg.Config(theCardinal));
	...
	case 'MNI/TALAIRACH',		%MNI/Talairach
		...
		if theCardinal>0,		
			...
			FG_rest_sliceviewer('SetPosition', AFigHandle);
		end;
		
	case 'CHANGEUNDERLAY', 		%ChangeUnderlay
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''ChangeUnderlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'UNDERLAYSELECTION', 		%UnderlaySelection	
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''UnderlaySelection'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,			
			...
			if any(filename~=0) && ischar(filename) && length(filename)>4 ,	% not canceled and legal			
				...
				FG_rest_sliceviewer('ChangeUnderlay', AFigHandle);
			end			
		end;
	case 'CLICKRECENTUNDERLAY', 	%ClickRecentUnderlay
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''ClickRecentUnderlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,			
	...
	case 'CHANGEOVERLAY', 		%ChangeOverlay
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''ChangeOverlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
			...
			% if ~all(isspace(theNewOverlay)),
                ...
                REST_SliceViewer_Cfg.Config(theCardinal) =LoadOverlay(REST_SliceViewer_Cfg.Config(theCardinal), theNewOverlay);
                    REST_SliceViewer_Cfg.Config(theCardinal).Overlay.ValueThrdAbsolute = PtoThrd(0.001,REST_SliceViewer_Cfg.Config(theCardinal));%% cliff manually add, this 0.001 is corresponding to the 0.001 in line 472
                    REST_SliceViewer_Cfg.Config(theCardinal) =ThresholdOverlayVolume(REST_SliceViewer_Cfg.Config(theCardinal));%% cliff manually add after adding line 444
                % YAN Chao-Gan and DONG Zhang-Ye, 100401.
	...
	case 'OVERLAYSELECTION', 		%OverlaySelection	
		...
		if theCardinal>0,		
			...
			if any(filename~=0) && ischar(filename) && length(filename)>4 ,	% not canceled and legal			
                ...
                set(REST_SliceViewer_Cfg.Config(theCardinal).hEdtPValue, 'String', '0.001');  %DONG 100118 change the init P value  %%cliff corresponding to line444/445's 0.001
				FG_rest_sliceviewer('ChangeOverlay', AFigHandle);
	...
	case 'CLICKRECENTOVERLAY', 	%ClickRecentOverlay
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''ClickRecentOverlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,			
	...
	case 'OVERLAY_SETTHRDABSVALUE',		%Overlay_SetThrdAbsValue, %Change from the Absolute value Edit control
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''Overlay_SetThrdAbsValue'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'OVERLAY_SETTHRDCLUSTERSIZE', 		%Overlay_SetThrdClusterSize
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''Overlay_SetThrdClusterSize'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'OVERLAY_MISC', 		%Overlay_Misc
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''Overlay_Misc'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
			REST_SliceViewer_Cfg.Config(theCardinal) =Overlay_Misc(REST_SliceViewer_Cfg.Config(theCardinal));
			FG_rest_sliceviewer('Repaint', AFigHandle);
		end	
    ...
    case 'CLUSTERSREPORT'
        if nargin~=2, error('Usage: FG_rest_sliceviewer(''Clusters Report'', AFigHandle);'); end
	...
	case 'CURRENTTHRD2MASK',	%dong save thrd2mask 2009-09-09
        if nargin~=2, error('Usage: FG_rest_sliceviewer(''CurrentThrd2Mask'', AFigHandle);'); end
        AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
       if theCardinal>0,
			theConfig =REST_SliceViewer_Cfg.Config(theCardinal);
	...
	case 'CURRENTCLUSTER2MASK',			%CurrentCluster2Mask
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''CurrentCluster2Mask'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
			theConfig =REST_SliceViewer_Cfg.Config(theCardinal);
	...
	case 'SAVERECENT',		%SaveRecent		
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''SaveRecent'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'CHANGECOLORELEMENT', 		%ChangeColorElement
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''ChangeColorElement'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'TOGGLEINFODISPLAY', 		%ToggleInfoDisplay
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''ToggleInfoDisplay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'UPDATECALLBACK_SAVE2MASK',	%UpdateCallback_Save2Mask
		if nargin~=3, error('Usage: FG_rest_sliceviewer(''UpdateCallback_Save2Mask'', AFigHandle, ACallback);'); end
		AFigHandle 	=varargin{1};	
		ACallback   =varargin{2};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'GETSAVEDMASKFILENAME',		%GetSavedMaskFilename
		if nargin~=2, error('Usage: FG_rest_sliceviewer(''GetSavedMaskFilename'', AFigHandle);'); end
		AFigHandle 	=varargin{1};			
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'SHOWOVERLAY',	%ShowOverlay
		if nargin>3, error('Usage: FG_rest_sliceviewer(''ShowOverlay'', AOverlay); or FG_rest_sliceviewer(''ShowOverlay'', AFigHandle, AOverlay);'); end
		AFigHandle 	=varargin{1};
		if ~ischar(AFigHandle),
			...
			AOverlay    =varargin{1};
			theFig = FG_rest_sliceviewer; %I can't write "()" considering Matlab 6.5 compatiable
			FG_rest_sliceviewer('ShowOverlay', theFig, AOverlay);
			varargout{1} =theFig;
		end
        
        case 'ONKEYPRESS',		%OnKeyPress
            if nargin~=2, error('Usage: FG_rest_sliceviewer(''OnKeyPress'', AFigHandle);'); end
            AFigHandle 	=varargin{1};
            theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
            if theCardinal>0,
        ...
        case 'SCROLLWHEELDOWN',		%dong 100520
            if nargin~=2, error('Usage: FG_rest_sliceviewer(''ScrollWheel'', AFigHandle);'); end
            AFigHandle 	=varargin{1};
            theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
            if theCardinal>0,
...
% catch
% 	FG_rest_misc( 'DisplayLastException');
% end	
...
function Result =InitControls(AFilename, ACallback)	
	...
	set(theFig, 'DeleteFcn', sprintf('FG_rest_sliceviewer(''Delete'', %g);', theFig)  );
	set(theFig, 'KeyPressFcn', sprintf('FG_rest_sliceviewer(''OnKeyPress'', %g);', theFig)  );
    ...
    %if FG_rest_misc('GetMatlabVersion')>=7.4%100520 dong Matlab 7.4 begin
	...
	hYoke =uicontrol(theFig, 'Style','checkbox', 'Units','pixels', ...
					'String', 'Yoke', 'Value', 1,... % cliff good, original:  ['String', 'Yoke', ...] 
					'BackgroundColor', get(theFig,'Color'), ...
	...
	hCrosshair =uicontrol(theFig, 'Style','checkbox', 'Units','pixels', ...
					...
					'Callback', sprintf('FG_rest_sliceviewer(''Repaint'', %g);',theFig) , ...
					'BackgroundColor', get(theFig,'Color'), ...
					'Position',[theLeft, theBottom+75, 70,15]);
	
	
	theEditCallbackFcn =sprintf('FG_rest_sliceviewer(''SetPosition'', %g);', theFig);
	...
	hMagnify =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
						...
						'Value', 2, ... % cliff original 2
						'BackgroundColor', get(theFig,'Color'), ...
						'Enable', 'off', ...
						'Callback', sprintf('FG_rest_sliceviewer(''Magnify'', %g);',theFig), ...
	...
	hMniTal =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
						...
						'BackgroundColor', get(theFig,'Color'), ...
						'Enable', 'on', ...
						'Callback', sprintf('FG_rest_sliceviewer(''MNI/Talairach'', %g);',theFig), ...
	...
	hMsgLabel =uicontrol(theFig, 'Style','text', 'Units','pixels', ...
			  ...
			  'ButtonDownFcn', sprintf('FG_rest_sliceviewer(''ToggleInfoDisplay'', %g);',theFig), ...
	...
	theIcon =imread(fullfile(FG_rest_misc( 'WhereIsREST'), 'icoTransverse.jpg'));
	hViewTransverse =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('FG_rest_sliceviewer(''TransverseView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	theLeft =OffsetX+MarginX+30; theBottom =OffsetY +MarginY-2;
	theIcon =imread(fullfile(FG_rest_misc( 'WhereIsREST'), 'icoSagittal.jpg'));
	hViewSagittal =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('FG_rest_sliceviewer(''SagittalView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	theLeft =OffsetX+MarginX+34+30; theBottom =OffsetY +MarginY-2;
	theIcon =imread(fullfile(FG_rest_misc( 'WhereIsREST'), 'icoCoronal.jpg'));
	hViewCoronal =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('FG_rest_sliceviewer(''CoronalView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	theLeft =OffsetX+MarginX+34+34+30; theBottom =OffsetY +MarginY-2;
	theIcon =imread(fullfile(FG_rest_misc( 'WhereIsREST'), 'icoOrthogonal.jpg'));
	hViewOrthogonal =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('FG_rest_sliceviewer(''OrthogonalView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	theLeft =OffsetX+MarginX+34+30; theBottom =OffsetY +MarginY+34;	
	hViewMontage =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('FG_rest_sliceviewer(''Montage'', %g);',theFig) , ...
	...
	hUnderlayRecent =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
			  ...
			  'Callback', sprintf('FG_rest_sliceviewer(''ClickRecentUnderlay'', %g);',theFig), ...
			  'BackgroundColor', get(theFig,'Color'), ...			  
	...
	hUnderlayFile =uicontrol(theFig, 'Style','edit', 'Units','pixels', ...
							  'String', '', ...
							  'BackgroundColor', 'white', ...
							  'Callback', sprintf('FG_rest_sliceviewer(''ChangeUnderlay'', %g);',theFig), ...
							  'Position',[theLeft, theBottom, 150,20]);
	theLeft =OffsetX+116; theBottom =MarginY+225;
	uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...
		  'Callback', sprintf('FG_rest_sliceviewer(''UnderlaySelection'', %g);',theFig) , ...
	...
	hOverlayRecent =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
			  ...
			  'Callback', sprintf('FG_rest_sliceviewer(''ClickRecentOverlay'', %g);',theFig), ...
			  'BackgroundColor', get(theFig,'Color'), ...			  
			  'Position',[theLeft, theBottom, 75,20]);	 
	...
	hOverlayFile =uicontrol(theFig, 'Style','edit', 'Units','pixels', ...
					  'String', '', ...					  
					  'BackgroundColor', 'white', ...
					  'Callback', sprintf('FG_rest_sliceviewer(''ChangeOverlay'', %g);',theFig), ...
					  'Position',[theLeft, theBottom, 150,20]);
	theLeft =OffsetX+116; theBottom =MarginY+185;
	uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...
		  'Callback', sprintf('FG_rest_sliceviewer(''OverlaySelection'', %g);',theFig) , ...
		  'String', '...','FontWeight', 'bold', ...
		  'Position',[theLeft, theBottom, 34,15]); 
	...
	hEdtThrdValue =uicontrol(theFig, 'Style','edit', 'Units','pixels', ...
					'String', '', ...					
					'Callback', sprintf('FG_rest_sliceviewer(''Overlay_SetThrdAbsValue'', %g);',theFig), ...	
					 'BackgroundColor', 'white', ...
   ...
   uicontrol(theFig, 'Style','text', 'Units','pixels', ...%dong
			'String', 'P', ...								
			 'BackgroundColor', get(theFig,'Color'), ...
   ...
   hEdtPValue =uicontrol(theFig, 'Style','edit', 'Units','pixels', ...%P-value need improve dong
    ...
    hdf =uicontrol(theFig, 'Style','pushbutton',  ... %add df dong 090921
							  'Units','pixels', ...
							  'Callback', sprintf('FG_rest_sliceviewer(''DF'', %g);',theFig),...	
							  'String', 'df', ...
...
uicontrol(theFig, 'Style','text', 'Units','pixels', ...%dong
     ...
     uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...  %only + dong 090921
			'String', '+', ...
			'Callback', sprintf('FG_rest_sliceviewer(''OnlyPositive'', %g);',theFig),...			
			'Position',[theLeft+112, theBottom+80, 15,18]);
     uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...  %only + dong 090921
			'String', '-', ...
			'Callback', sprintf('FG_rest_sliceviewer(''OnlyNegative'', %g);',theFig),...			
			'Position',[theLeft+129, theBottom+80, 15,18]);
        
	uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...
			'String', 'Cluster Size', ...
			'Callback', sprintf('FG_rest_sliceviewer(''Overlay_SetThrdClusterSize'', %g);',theFig),...		%here dong	
	...
	hOverlayMisc =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
					...
					'BackgroundColor', get(theFig,'Color'), ...
					'Enable', 'on', ...
					'Callback', sprintf('FG_rest_sliceviewer(''Overlay_Misc'', %g);',theFig), ...
	...
	hTemplate =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
					...
					'BackgroundColor', get(theFig,'Color'), ...
					'Enable', 'on', ...
					'Callback', sprintf('FG_rest_sliceviewer(''Open_Template'', %g);',theFig), ...
					'Position',[theLeft+75, theBottom+55, 70,24]);
	uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...
			'String', 'Save Cluster', ...
			'Callback', sprintf('FG_rest_sliceviewer(''CurrentCluster2Mask'', %g);',theFig),...			
			'Position',[theLeft+75, theBottom+30, 70,24]);
   uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...%dong 090920
			'String', 'Save Clusters', ...
			'Callback', sprintf('FG_rest_sliceviewer(''CurrentThrd2Mask'', %g);',theFig),...			
			'Position',[theLeft+5, theBottom+5, 70,24]);
    uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...%dong 090920
			'String', 'Cl. Report', ...
           'Callback', sprintf('FG_rest_sliceviewer(''ClustersReport'', %g);',theFig),...				
			'Position',[theLeft+75, theBottom+5, 70,24]);
	% uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...
	%		 'String', 'Thrd2Mask', ...
	%		 'Callback', sprintf('FG_rest_sliceviewer(''Overlay_SetThrdClusterSize'', %g);',theFig),...			
	%		 'Position',[theLeft+75, theBottom+5, 70,24]);
			
	%Colorbar for overlay, do as AFNI, 20070921
	theAxesButtonDownFcn =sprintf('FG_rest_sliceviewer(''ChangeColorElement'', %g);',theFig);
	...
	%%%% AConfig.Filename =AFilename;	%Default for underlay   % cliff
    AConfig.Filename =fullfile(FG_rest_misc( 'WhereIsREST'), 'Template', 'ch2bet.nii'); % cliff good, original: AConfig.Filename =AFilename
	...
	AConfig.Montage.Across =10; % cliff, orginal 1
	AConfig.Montage.Down   =1; % cliff, orginal 1
	AConfig.Montage.Spacing=10; % cliff, orginal 3
	...
	%Options for Overlay	
	AConfig.Overlay.Colormap	=AFNI_ColorMap(12);  % cliff good,original: jet(64)
	AConfig.Overlay.ColorbarCmd	='12'; % cliff good,original: 'jet(64)'
	...
	AConfig.Overlay.ClusterSizeThrd =20; %Default, 0 voxels, for not confining cluster-size % cliff good,original: 0
	AConfig.Overlay.ClusterRadiusThrd =0; %Default radius(mm) for Cluster size definition
    AConfig.Overlay.ClusterConnectivityCriterion =18; %Default Connectivity Criterionradius: 0.  % cliff good,original: 0, corresponding to the next one. refer to BWLABEL
    AConfig.Overlay.ClusterConnectivityCriterionRMM=5;  % cliff good,original: 0
	...
	% if FG_rest_misc('GetMatlabVersion')>=7.3
...
function Result =DeleteFigure(AGlobalConfig, AFigHandle)
	...
	if x>0,			
		...
		isExistFig =FG_rest_misc( 'ForceCheckExistFigure' , AGlobalConfig.Config(x).hFig);
		if isExistFig,
			%Save the recent menu
			FG_rest_sliceviewer('SaveRecent', AFigHandle);
...
function Result =SetImage(AConfig)
	...
	set(AConfig.hFig, 'Name', sprintf('Slice Viewer -- REST %s         %s',FG_rest_misc( 'GetRestVersion'),AConfig.Overlay.Filename));
...
function Result =SetView_Orthogonal(AConfig)	
	...
	if license('test','image_toolbox')==1 && theMagnifyCoefficient~=1, 
        FullMatlabVersion = sscanf(version,'%d.%d.%d.%d%s');
        if FullMatlabVersion(1)*1000+FullMatlabVersion(2)>=7*1000+4    %YAN Chao-Gan, 101025. Fixed a bug when displaying in MATLAB 2010.
        %if FG_rest_misc('GetMatlabVersion')>=7.4   %YAN Chao-Gan 090401: The imresize function has been completely rewritten in Matlab R2007a. Fixed the bug of 'Set Overlay's Color bar' in Matlab R2007a or latter version.
...
function SetMessage(AConfig)
	...
	if SeeOverlay(AConfig),
		...
		if all(theOverlayPos<=size(AConfig.Overlay.Volume)) ...
			...
			FG_rest_sliceviewer('SetPhysicalPosition', AConfig.hFig, [0 0 0]);
			FG_rest_sliceviewer('Repaint', AConfig.hFig);
			return;
		end
...
function Result =GetMagnifyCoefficient(AConfig)	
	...
	case 'X3',
		Result =3;
	otherwise
		FG_rest_misc( 'ComplainWhyThisOccur');
	end
...
function Result =UpdateCallback(AConfig, ACallback, ACallbackCaption)
	...
	% else
		...
		% if ~isempty(ACallback) && (ischar(ACallback) || isa(ACallback, 'function_handle')),
			% theCallback =sprintf('FG_rest_sliceviewer(''RunCallback'', ''%s'');',AConfig.Filename);
...
function Transforming_MNI_Talairach(AConfig)
	...
	if isNeedUpdate,
		ShowPositionInEdit(AConfig);
		SetMessage(AConfig);
		FG_rest_sliceviewer('SetPosition', AConfig.Filename);
	end
...
%Input: Need AConfig.Filename specified	
	try
		[theVolume,theVoxelSize, Header] =FG_rest_readfile(AConfig.Filename);  %%Yan 080610
	...
	theAxesButtonDownFcn =sprintf('FG_rest_sliceviewer(''ClickPosition'', %g);', AConfig.hFig);
...
%By default, I only set the Min absolute value to be the threshold, and renturn the Min, 20070918
	...
	if strcmpi(get(theObject, 'Style'), 'edit'), %dong 090921 add p value input
        ...
        if edtPosition(1) ==85 
            ...
            Pvalue=ThrdtoP(Result,AConfig);  % cliff good function
        else
            Pvalue =str2num(get(AConfig.hEdtPValue, 'String'));
            Result =PtoThrd(Pvalue,AConfig);  % cliff good function
        end
...
%Such as AAL, BRODMANN ...	
	...
	[pathstr, name, ext] = fileparts(AConfig.Overlay.Filename);
	if strcmpi(name, 'aal'),		
		AColorMap =rest_ReadLutColorScheme(fullfile(FG_rest_misc( 'WhereIsREST'),'Template', 'aal.nii.lut'));	%YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	 
	...
	elseif strcmpi(name, 'brodmann'),		
		AColorMap =rest_ReadLutColorScheme(fullfile(FG_rest_misc( 'WhereIsREST'),'Template', 'brodmann.nii.lut'));		%YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	  
		theBlackPos=sum(AColorMap, 2)==0;
		AColorMap(theBlackPos, :) =1; %Black to White for I use black always as the background
		Result =overlay_ind2rgb(1+floor(Result), AColorMap);%Overpass the first color which may be backgound color	
	end
...
function Result =LoadOverlay(AConfig, AFilename)
	...
	Result =AConfig;
	try
		[theVolume,theVoxelSize, Header] =FG_rest_readfile(AFilename); %%Yan 080610
        theOrigin=Header.Origin; %%Yan 080610
	...
	Result.Overlay.ValueThrdAbsolute =theAbsMin; %Default, show all  %% cliff 
	...
	if strcmpi(name, 'aal'),		
		theInfoTxt =fullfile(FG_rest_misc( 'WhereIsREST'),'Template', 'aal.nii.txt'); %YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	 
...
%                 },'Correction Thresholds by AlphaSim');
        ...
        case 9 % DONG FDR 100118
            ...
            if ~isempty(AConfig.Overlay.Qmaskname)
                [MaskData MaskVox MaskHead]=FG_rest_file(AConfig.Overlay.Qmaskname);
...
function Result =Open_Template(AConfig)	
	...
	case 2, %'Open Template AAL'
		FG_rest_misc( 'CheckTemplate');
		theNewOverlay =fullfile(FG_rest_misc( 'WhereIsREST'), 'Template', 'aal.nii'); %YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	 
		set(AConfig.hOverlayFile, 'String', theNewOverlay);
		% FG_rest_sliceviewer('ChangeOverlay', AConfig.hFig);
		AConfig =LoadOverlay(AConfig, theNewOverlay);
		set(AConfig.hSeeOverlay, 'Value', 1);
	case 3, %'Open Template Brodmann'
		FG_rest_misc( 'CheckTemplate');
		theNewOverlay =fullfile(FG_rest_misc( 'WhereIsREST'), 'Template', 'brodmann.nii'); %YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	 
		set(AConfig.hOverlayFile, 'String', theNewOverlay);
		% FG_rest_sliceviewer('ChangeOverlay', AConfig.hFig);			
		AConfig =LoadOverlay(AConfig, theNewOverlay);
		set(AConfig.hSeeOverlay, 'Value', 1);
	case 4, %'Open Template Ch2'
		FG_rest_misc( 'CheckTemplate');
		theNewUnderlay =fullfile(FG_rest_misc( 'WhereIsREST'), 'Template', 'ch2.nii'); %YAN Chao-Gan 100403: use the ch2 bet image information from the MRIcroN	 
	...
	case 5, %'Open Template Ch2'
		FG_rest_misc( 'CheckTemplate');
		theNewUnderlay =fullfile(FG_rest_misc( 'WhereIsREST'), 'Template', 'ch2bet.nii'); %YAN Chao-Gan 100403: use the ch2 bet image information from the MRIcroN	 
		set(AConfig.hUnderlayFile, 'String', theNewUnderlay);
		% FG_rest_sliceviewer('ChangeOverlay', AConfig.hFig);	
		AConfig.Filename =theNewUnderlay;
		AConfig =InitUnderlay(AConfig);			
...
function Result =InitRecent(AConfig)
	Result =AConfig;	 
	theRecentCfg =fullfile(tempdir,['RecentUnderlay','_',FG_rest_misc('GetCurrentUser'),'.txt']); %YAN Chao-Gan, 100420.  %theRecentCfg =fullfile(FG_rest_misc( 'WhereIsREST'), 'RecentUnderlay.txt');
	if  exist(theRecentCfg, 'file')==2,
		[Result.Recent.Underlay] =textread(theRecentCfg,'%s', 'delimiter','\n');
	else
		Result.Recent.Underlay={};
    end
	theRecentCfg =fullfile(tempdir,['RecentOverlay','_',FG_rest_misc('GetCurrentUser'),'.txt']);  %YAN Chao-Gan, 100420. %theRecentCfg =fullfile(FG_rest_misc( 'WhereIsREST'), 'RecentOverlay.txt');
	if  exist(theRecentCfg, 'file')==2,
...
%AType = RecentUnderlay.txt or RecentOverlay.txt
	theRecentCfg =fullfile(tempdir,[AType,'_',FG_rest_misc('GetCurrentUser'),'.txt']);  % YAN Chao-Gan, 100420. %theRecentCfg =fullfile(FG_rest_misc( 'WhereIsREST'), [AType, '.txt']);
...
if (evnt.VerticalScrollCount>0),
    % scroll down
    eval(sprintf('FG_rest_sliceviewer(''ScrollWheelDown'', %g);', src));
else
    % scroll up
    eval(sprintf('FG_rest_sliceviewer(''ScrollWheelUp'', %g);', src));
end;
...
for mm = clusterID
    ...
    % cliff marked, make the xjview report only one pixel in a cluster
 %   if size(peakpos,1)>1
 %      peakpos = peakpos(1,:) ;
 %   end
