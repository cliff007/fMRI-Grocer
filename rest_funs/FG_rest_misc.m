function varargout=FG_rest_misc(AOperation, varargin)	
...
case 'ISEXISTFIGURE', 						%IsExistFigure
	if nargin~=2, error('Usage: result =FG_rest_misc( ''IsExistFigure'' , AFigureHandle);'); end	
	if nargin>0,
		varargout{1} =IsExistFigure(varargin{1});
	end	
	
case 'FORCECHECKEXISTFIGURE', 	%  ForceCheckExistFigure
	if nargin~=2, error('Usage: result =FG_rest_misc( ''ForceCheckExistFigure'' , AFigureHandle);'); end	
	if nargin>0,
		varargout{1} =ForceCheckExistFigure(varargin{1});
	end	
	
case 'COMPLAINWHYTHISOCCUR', 	%ComplainWhyThisOccur	
	if nargin~=1, error('Usage: FG_rest_misc( ''ComplainWhyThisOccur'');'); end
...
case 'REPLACESINGLEQUOTA', 		% Replace single ' in filename or dirname to make sure the legal string used in sliceViewer and waveGraph
	if nargin~=2, error('Usage: FG_rest_misc( ''ReplaceSingleQuota'', AFilename);'); end	
	AFilename =varargin{1};
	varargout{1} = regexprep(AFilename, '(''{1})', '''''');
	
case 'DISPLAYLASTEXCEPTION'	, %DisplayLastException, 20070530
	if nargin~=1, error('Usage: FG_rest_misc( ''DisplayLastException'');'); end
	if nargout>0,
...
case 'GETDATETIMESTR', 	% GetDateTimeStr	
	if nargin~=1, error('Usage: FG_rest_misc( ''GetDateTimeStr'');'); end
	varargout{1} =GetDateTimeStr;
case 'GETDATESTR', 	% GetDateStr	
	if nargin~=1, error('Usage: FG_rest_misc( ''GetDateStr'');'); end
	varargout{1} =GetDateStr;	
	
case 'GETMATLABVERSION',		%GetMatlabVersion
	if nargin~=1, error('Usage: FG_rest_misc( ''GetMatlabVersion'');'); end
	varargout{1} =GetMatlabVersion;
	
case 'GETCURRENTUSER', 	%GetCurrentUser	
	if nargin~=1, error('Usage: FG_rest_misc( ''GetCurrentUser'');'); end
...
case 'GETRESTVERSION', 		%GetRestVersion
	if nargin~=1, error('Usage: FG_rest_misc( ''GetRestVersion'');'); end
...
case 'WHEREISREST', 		%WhereIsREST
	if nargin~=1, error('Usage: FG_rest_misc( ''WhereIsREST'');'); end
	[pathstr, name, ext] = fileparts(mfilename('fullpath'));
	varargout{1} =pathstr;
...
case 'UNLOCKMFILEINMEMORY',		%UnLockMFileInMemory	
	if nargin~=2, error('Usage: FG_rest_misc( ''UnLockMFileInMemory'', ''MFilename'');'); end
...
case 'UNLOCKRESTFILES', 		%UnlockRestFiles
	if nargin~=1, error('Usage: FG_rest_misc( ''UnlockRestFiles'');'); end	
...
case 'CLEARTEMPFILES', 		%ClearTempFiles, Clear temp files in temp dir generated by REST(such as fc.m, alff.m, rest_bandpass.m ...)	
	...
	if nargin~=1, error('Usage: FG_rest_misc( ''ClearTempFiles'');'); end
...
case 'CHECKTEMPLATE', 		%CheckTemplate, Extract AAL & Brodmann gz file	
	if nargin~=1, error('Usage: FG_rest_misc( ''CheckTemplate'');'); end
	path =FG_rest_misc( 'WhereIsREST');
	if ~(exist(fullfile(path,'Template','aal.nii'), 'file')==2), %Yan Chao-Gan 081223: check the NIFTI templates.
		...
		if ~(exist(theGZfile, 'file')==2),
			...
			FG_rest_misc( 'ExtractGZ', theGZfile, fullfile(path,'Template'));
		end
	end
...
case 'EXTRACTGZ',	%ExtractGZ, Extract gz cmpressed file to specified dir
	if nargin~=3, error('Usage: FG_rest_misc( ''ExtractGZ'', AGZFile, ADestDir);'); end
	AGZFile =varargin{1};
	ADestDir=varargin{2};
	gunzip(AGZFile, ADestDir);
	
case 'EXPORTCELLS2TXT',	%ExportCells2Txt
	if nargin~=3, error('Usage: FG_rest_misc( ''ExportCells2Txt'', ACellStruct, AFilename);'); end
...
case 'IMPORTLINESFROMTXT',	%ImportLinesFromTxt
	if nargin~=2, error('Usage: [Lines]=FG_rest_misc( ''ImportLinesFromTxt'', AFilename);'); end	
	AFilename	=varargin{1};
	varargout{1} =textread(AFilename,'%s', 'delimiter','\n');
	
case 'VIEWROI', 	%ViewROI
	if nargin~=2, error('Usage: FG_rest_misc( ''ViewROI'', AROIDef);'); end	
	AROIDef	=varargin{1};
	ViewROI(AROIDef);

case 'SETFIGVIEWSTYLE',		%SetFigViewStyle	%Still Need more work! 20071103
	if nargin~=2, error('Usage: FG_rest_misc( ''SetFigViewStyle'', AFigHandle);'); end	
	AFigHandle =varargin{1};
	%'FontName', 'Fixed', ...
	SetFigViewStyle(AFigHandle);

case 'ATTENTION_COORDINATES',		%Attention_Coordinates
	if nargin~=1, error('Usage: FG_rest_misc( ''Attention_Coordinates'');'); end	
	...
	if rest_SphereROI( 'IsBallDefinition', AROIDef)
		...
		try
			...
			if any(filename~=0) && ischar(filename) && length(filename)>4 ,	% not canceled and legal					
				...
				[theOneTimePoint VoxelSize, Header] = FG_rest_readfile(theBrainMap);
				BrainSize =size(theOneTimePoint);
				
				[AROICenter, AROIRadius] =rest_SphereROI('STR2ROIBALL', AROIDef);
				rest_SphereROI( 'BallDefinition2Mask' , AROIDef, BrainSize, VoxelSize, Header);
				
				theViewer =rest_sliceviewer('ShowImage', theBrainMap);
				rest_sliceviewer('ShowOverlay',theViewer, fullfile(tempdir,['LastSphereMask_',FG_rest_misc('GetCurrentUser')]));  %Revised by YAN Chao-Gan, 091126. LastSphereMask would be stored under temp dir other than {REST_DIR}. %rest_sliceviewer('ShowOverlay',theViewer, fullfile(FG_rest_misc('WhereIsREST'),'LastSphereMask'));
