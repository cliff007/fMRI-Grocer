function bandfiltered_full_names = FG_rest_bandpass(ADataDir,fun_imgs, ...
...
% 	[AllVolume,vsize,theImgFileList, Header,nVolumn] =rest_to4d(ADataDir);
    theImgFileList=fun_imgs;
    Vmats=spm_vol(fun_imgs);
    AllVolume=spm_read_vols(Vmats);
    Header=Vmats(end);
    nVolumn=size(theImgFileList,1);
    vsize=sqrt(sum(Header.mat(1:3,1:3).^2));  %% cliff: calculate the voxel size from image's .mat field
                                        %% be careful:   b=abs(det(a.mat)) is to calculat the volume in cubic millimeters 
                                        %% L*W*H = abs(det(a.mat)), it is
                                        %% same as vsize(1)*vsize(2)*vsize(3)
	...
	theTempDatasetDirName =sprintf('BandPass_%d_%s', fix((1e4) *rem(now, 1) ),FG_rest_misc('GetCurrentUser'));	
...
% 	mask=rest_loadmask(nDim1, nDim2, nDim3, AMaskFilename);
    ...
    if isnumeric(AMaskFilename)  &&  AMaskFilename==1  % cliff
        mask=ones(nDim1, nDim2, nDim3);
    elseif ischar(AMaskFilename)
        fprintf('\n\t Loading default mask "%s".', AMaskFilename);	  % redundant code;
        try
            mask=FG_read_vols(AMaskFilename);
        catch me
            me.message
            return    
        end
...
% 	fprintf('\n\t Build band pass filtered mask.\tWait...');
	...
	paddedLength = FG_rest_nextpow2_one35(sampleLength); %2^nextpow2(sampleLength);   %% cliff
...
% 	%20070513	remove trend --> FFT --> filter --> inverse FFT --> retrend 
	...
	fprintf('\n\t Band Pass Filtering is working.........');		
...
if ~(exist(theResultOutputDir,'dir')==7) % cliff
    ...
    bandfiltered_full_names=[];
	for x=1:sampleLength
        new_file_name=sprintf('%s_filtered%s%.8d', ADataDir, filesep,x);
        ext=deblank(Header.fname);
        ext=ext(end-3:end);
		FG_rest_writefile(single(theDataset4D(:, :, :, x)), ...
        ...
        bandfiltered_full_names=strvcat(bandfiltered_full_names,[new_file_name ext]);
    end	    
    
	fprintf('\n-----------Band pass filter is over........\n');
