...
function FG_voxel_coord_AAL_region_inqury
    file = spm_select(1,'any','Select the reference img file that define the Voxel-space', [],pwd,'.*img$|.*nii$');
    M=spm_vol(file);
    T=M.mat;

    dlg_prompt={'x{voxel} ,y{voxel} ,z{voxel},,or you can just close this window to select a Coord. txt file'};
    dlg_name='Voxel region(voxel coordinate)';
    %% inquery one by one
  ...
  for cor_num=1:size(cor,1)   
    fprintf('voxel-space coord: %d% d %d \n',cor(cor_num,1),cor(cor_num,2),cor(cor_num,3))  
    mni=cor2mni(cor(cor_num,:),T);%,head.mat);
    pos =1;
    tmpmni = mni(pos,:);
