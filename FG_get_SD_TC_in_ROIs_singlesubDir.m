 function  FG_get_SD_TC_in_ROIs_singlesubDir
      ...
      for k=k_low:k_high,
         ...
         % get the original cbf img's global mean cbf value using "spm_global()" based on SPM caculate the gray-matter's global mean cbf
           ...
           % this can't handle the voxels that really have value of "0"
               ...
               global_mean=std(within_mask_vaild_values); % get the std of all the values that is Non-Nan within the mask (include all vaild '0's if there is some)  
        ...
        write_name2=FG_check_and_rename_existed_file(fullfile(pth,'ROIs_for_STD_TCs.csv'));
        % if you want to see the sequence number of each ROI in the output file, use these lines
        if p==1
            write_name1=FG_check_and_rename_existed_file(fullfile(pth,'ROIs_names_for_STD.txt'));
