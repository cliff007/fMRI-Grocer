...
%  Superior.
%  
%  Usage: nii = FG_load_nii(filename, [img_idx], [dim5_idx], [dim6_idx], ...
...
%
function nii = FG_load_nii(filename, img_idx, dim5_idx, dim6_idx, dim7_idx, ...
...
if ~exist('filename','var')
   ...
   if ~exist('filename','var')
      error('Usage: nii = FG_load_nii(filename, [img_idx], [dim5_idx], [dim6_idx], [dim7_idx], [old_RGB], [tolerance], [preferredForm])');
   end
...
%   nii.ext = FG_load_nii_ext(filename);
   ...
   %
   [nii.img,nii.hdr] = FG_load_nii_img(nii.hdr,nii.filetype,nii.fileprefix, ...
		nii.machine,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB);

   %  Perform some of sform/qform transform
   %
   nii = FG_xform_nii(nii, tolerance, preferredForm);
