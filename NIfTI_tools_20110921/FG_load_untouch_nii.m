...
%
function nii = FG_load_untouch_nii(filename, img_idx, dim5_idx, dim6_idx, dim7_idx, ...
			old_RGB, slice_idx)
...
if ~exist('filename','var')
   ...
   if ~exist('filename','var')
      error('Usage: nii = FG_load_untouch_nii(filename, [img_idx], [dim5_idx], [dim6_idx], [dim7_idx], [old_RGB], [slice_idx])');
   end
   ...
   %
   [nii.hdr,nii.filetype,nii.fileprefix,nii.machine] = FG_load_nii_hdr(filename);

   if nii.filetype == 0
      nii.hdr = FG_load_untouch0_nii_hdr(nii.fileprefix,nii.machine);
      nii.ext = [];
   else
      nii.hdr = FG_load_untouch_nii_hdr(nii.fileprefix,nii.machine,nii.filetype);

      %  Read the header extension
      %
      nii.ext = FG_load_nii_ext(filename);
   end

   %  Read the dataset body
   %
   [nii.img,nii.hdr] = FG_load_untouch_nii_img(nii.hdr,nii.filetype,nii.fileprefix, ...
