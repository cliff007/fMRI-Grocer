...
%
%  Usage:  	status = FG_view_nii([h], nii, [option])	or
%		status = FG_view_nii(h, [option])
%
...
%
function status = FG_view_nii(varargin)    
   if nargin < 1
      error('Please check inputs using ''help FG_view_nii''');
   end;
   ...
   elseif ischar(varargin{1})		% call back by event
      ...
      FG_view_nii_menu(fig);
      FG_rri_file_menu(fig);
   ...
   case {'clearnii'}
      ...
      mymotion = '; FG_view_nii(''move_cursor'');';
...
%      highcolor = [zeros(1,3);gray(55)];
   ...
   if xdim > 1,
      ...
      handles.sagittal_slider = uicontrol('Parent',fig, ...
                ...
                'Callback','FG_view_nii(''sagittal_slider'');');
   ...
   if ydim > 1,
      ...
      handles.coronal_slider = uicontrol('Parent',fig, ...
                ...
                'Value',slider_val,...
                'Callback','FG_view_nii(''coronal_slider'');');
...
%   h = front_pos(4);
   ...
   if zdim > 1,
      ...
      handles.axial_slider = uicontrol('Parent',fig, ...
                ...
                'Callback','FG_view_nii(''axial_slider'');');
...
%   info_pos = info_pos(:);
   ...
   handles.impos = uicontrol('Parent',fig,'Style','edit', ...
        ...
        'BusyAction','queue',...
        'Callback','FG_view_nii(''impos_edit'');', ...
...
if 0
   ...
   handles.coord = uicontrol('Parent',fig,'Style','popupmenu', ...
      ...
      'visible','off', ...
      'Callback','FG_view_nii(''coordinates'');');
...
%      'String',{'Voxel','MNI (mm)','Talairach (mm)'},...
   ...
   handles.xhair_color = uicontrol('Parent',fig,'Style','push', ...
      ...
      'String','Color',...
      'visible','off', ...
      'Callback','FG_view_nii(''xhair_color'');');
   ...
   handles.xhair = uicontrol('Parent',fig,'Style','popupmenu', ...
      ...
      'String',{'On','Off'},...
      'visible','off', ...
      'Callback','FG_view_nii(''crosshair'');');
...
%      set(handles.hist_eq, 'visible', 'on');
   ...
   handles.hist_plot = uicontrol('Parent',fig,'Style','push', ...
      ...
      'String','Hist Plot',...
      'visible','off', ...
      'Callback','FG_view_nii(''hist_plot'');');
...
%      'String',{'Voxel','MNI (mm)','Talairach (mm)'},...
   ...
   handles.neg_color = uicontrol('Parent',fig,'Style','toggle', ...
      ...
      'String','Negative',...
      'visible','off', ...
      'Callback','FG_view_nii(''neg_color'');');
...
%      set(handles.Thist, 'visible', 'on');
   ...
   if nii_view.numscan > 1
      set(handles.contrast, 'min', 1, 'max', nii_view.numscan, ...
         ...
         'Callback', 'FG_view_nii(''slider_change_scan'');');
   ...
   handles.brightness = uicontrol('Parent',fig, ...
      ...
      'Value',0, ...
      'visible','off', ...
      'Callback','FG_view_nii(''brightness'');');
   ...
   handles.contrast_def = uicontrol('Parent',fig,'Style','push', ...
      ...
      'String','Reset',...
      'visible','off', ...
      'Callback','FG_view_nii(''contrast_def'');');
   ...
   if nii_view.numscan > 1
      set(handles.contrast_def, 'style', 'edit', 'background', 'w', ...
         ...
         'string', '1', 'Callback', 'FG_view_nii(''edit_change_scan'');');
   elseif colorindex < 2 | colorindex > 3
   ...
   handles.brightness_def = uicontrol('Parent',fig,'Style','push', ...
      ...
      'String','Reset',...
      'visible','off', ...
      'Callback','FG_view_nii(''brightness_def'');');
   ...
   h1 = plot_view(fig, xdim, ydim, top_ax, img_slice, clim, cbarminmax, ...
	handles, useimagesc, colorindex, color_map, ...
	colorlevel, highcolor, useinterp, nii_view.numscan);
   set(h1,'buttondown','FG_view_nii(''axial_image'');');
   ...
   h1 = plot_view(fig, xdim, zdim, front_ax, img_slice, clim, cbarminmax, ...
	handles, useimagesc, colorindex, color_map, ...
	colorlevel, highcolor, useinterp, nii_view.numscan);
   set(h1,'buttondown','FG_view_nii(''coronal_image'');');
   ...
   h1 = plot_view(fig, ydim, zdim, side_ax, img_slice, clim, cbarminmax, ...
	handles, useimagesc, colorindex, color_map, ...
	colorlevel, highcolor, useinterp, nii_view.numscan);
   set(h1,'buttondown','FG_view_nii(''sagittal_image'');');
...
if 0
   ...
   windowbuttonmotion = [windowbuttonmotion '; FG_view_nii(''move_cursor'');'];
...
function nii_view = update_nii_view(nii_view)
   ...
   nii_view.axi_xhair = ...
	FG_rri_xhair(p_axi, nii_view.axi_xhair, nii_view.handles.axial_axes);

   nii_view.cor_xhair = ...
	FG_rri_xhair(p_cor, nii_view.cor_xhair, nii_view.handles.coronal_axes);

   nii_view.sag_xhair = ...
	FG_rri_xhair(p_sag, nii_view.sag_xhair, nii_view.handles.sagittal_axes);
...
function update_useinterp(fig, useinterp)
   ...
   if ~isempty(nii_view.handles.axial_image)
      ...
      set(nii_view.handles.axial_image,'buttondown','FG_view_nii(''axial_image'');');
   end
   ...
   if ~isempty(nii_view.handles.coronal_image)
      ...
      set(nii_view.handles.coronal_image,'buttondown','FG_view_nii(''coronal_image'');');
   end
   ...
   if ~isempty(nii_view.handles.sagittal_image)
      ...
      set(nii_view.handles.sagittal_image,'buttondown','FG_view_nii(''sagittal_image'');');
   end
...
function update_scanid(fig, setscanid)
   ...
   FG_view_nii(fig, nii_view.nii.img, opt);
...
%   if isfield(nii_view, 'colorlevel') & ~isempty(nii_view.colorlevel)
   ...
   switch colorindex
   case {2}
      color_map = FG_bipolar(num_color, cbarminmax(1), cbarminmax(2), contrast);
   case {3}
