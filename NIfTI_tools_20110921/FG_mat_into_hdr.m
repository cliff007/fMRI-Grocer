...
%
%-------------------------------------------------------------------------
function FG_mat_into_hdr(files)
if nargin==0
    all_files = spm_select(inf,'any','Select all the .hdr that have .mat files', [],pwd,'.*hdr$');
    if FG_check_ifempty_return(all_files), return; end
    
%     mat_files = spm_select(inf,'any','Select all the .mat files corresponding to the imgs', [],pwd,'.*mat$');
%     if FG_check_ifempty_return(mat_files), return; end
%     
%     if size(all_files,1)~=size(mat_files,1)
%        fprintf('\nError: the number of imgs and .mat files is different. Please check it out!\n') 
%        return
%     end
end

for i=1:size(all_files,1)
   files=deblank(all_files(i,:));
   ...
   for i=1:length(file_lst)
      ...
      if process
         ...
         [h filetype fileprefix machine]=FG_loadnii_hdr(hdr);
         h.hist.qform_code=0;
         h.hist.sform_code=1;
         h.hist.srow_x=M(1,:);
         h.hist.srow_y=M(2,:);
