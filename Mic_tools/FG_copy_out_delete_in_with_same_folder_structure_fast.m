function FG_copy_out_delete_in_with_same_folder_structure_fast
...
%          [all_dirs,all_full_dirs]=FG_readsubfolders(root_dir);  %   FG_genpath
%          all_dirs=char(all_full_dirs)

        pause(0.5)

        display('----Copying folder structure...')
         if strcmp(h_op,'Copy out...') ||  strcmp(h_op,'Move out...') 
            ...
            FG_copy_folder_structure(root_dir,out_dir,'Copy full folder structure') ; % build up the a whole folder structure to make the following direct copy possiblefprintf('\nRunning.........\n ');
         end
