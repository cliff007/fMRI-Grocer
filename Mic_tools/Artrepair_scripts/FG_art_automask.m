function  [Y,varargout] = FG_art_automask(Image,Threshold,WriteVol,varargin)
...
if ( Threshold == -1 )   % Find a threshold that discards three outer faces.
    ...
    if nargin==3
        Tlow = fix(0.2*Yr);   % cliff: default:  Tlow = fix(0.2*Yr); Thigh = fix(0.4*Yr) to separate the vaild regions
        Thigh = fix(0.4*Yr);    
    elseif nargin==5
        Tlow = fix(varargin{1}*Yr);    % cliff: if use higher probability (e.g. 0.4,0.8) to reduce the potential ouput area
        Thigh = fix(varargin{2}*Yr);         
    end
    Tskip = max(fix(Tlow/20),1);
    for Tbar = Tlow:Tskip:Thigh   % 400:20:800
        temp(:,:,:) = (Y(:,:,:) > Tbar);  % cliff: temp will be the selected output volume (Y) corresponding to a Tbar after the loop
	% Count the number of mask points in the far faces of the volume
         xdim = size(Y);
         count1 = sum(sum(temp(:,:,1))); % cliff, first plane along z-axis
         count2 = sum(sum(temp(:,:,xdim(3))));   % cliff, last plane along z-axis
         count3 = sum(sum(temp(:,1,:))); % cliff, first plane along y-axis
         count4 = sum(sum(temp(:,xdim(2),:)));   % cliff, last plane along y-axis
         count5 = sum(sum(temp(1,:,:))); % cliff, first plane along x-axis
         count6 = sum(sum(temp(xdim(1),:,:)));   % cliff, last plane along x-axis
     % Always have one face with large counts, sometimes have 2 such faces.
         ...
         if ( countC < FMR*nvox )  % cliff, FMR=0.015= 1.5% is the predifined false mask percentage, we treat 3D voxels-planes that have voxel numbers<15% as the voxels out of the brain 
...
%         if Threshold == -1
%             if Tbar >= Thigh-Tskip
%                 fprintf('\n--Automask program failed.Try choosing a mean image, or manually set a threshold. Type help art_automask...\n')
%                 Y=temp;
%                 varargout{1}=[];  %% cliff
%                 return
%             end
%         end
    ...
    if Threshold == -1
        last_end=Thigh-Tskip;
        if Tbar >= last_end
            fprintf('\n--Automask program is failed. Nothing change...\n')
            Y=V.fname;
            varargout{1}=[];  %% cliff
            return
        end
 ...
 % If the edge of a plane has much more data than the center, delete the edge. 
 % Then if any plane has too few data points (meaning close to noise level)
 % then set the entire plane in the mask image to zero. 
 % Note for spiral scans: a better idea might be to determine the orientation
 % of the spiral, and just remove the potential alias effects at those four edges corners.
        xdim = size(Y);
...
% Outputs
Y = temp;  % cliff: temp now is  the selected output volume (Y) corresponding to a Tbar after edge cleaning
    
if ( WriteVol == 1 )     
    v = V;  % preserves the header structure
    [dirname, xname, xext ] = fileparts(V.fname);
    artifname = ['Ref_auto_Mask_' xname  xext];
    if nargin == 0
    ...
    Y=v.fname; % if write out the mask, then the Y-output is the path of the mask file
end
