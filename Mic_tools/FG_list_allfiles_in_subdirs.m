function [varargout] = FG_list_allfiles_in_subdirs(rootdir,varargin)
% RDIR - Recursive directory listing
% 
%  d = FG_list_allfiles_in_subdirs(ROOT)
%  d = FG_list_allfiles_in_subdirs(ROOT, TEST)
%  d = FG_list_allfiles_in_subdirs(ROOT, TEST, RMPATH)
%  d = FG_list_allfiles_in_subdirs(ROOT, TEST, 1)
%  d = FG_list_allfiles_in_subdirs(ROOT, '', ...)
%  FG_list_allfiles_in_subdirs(...)
%
%
% *Inputs*
%
% * ROOT
%
% FG_list_allfiles_in_subdirs(ROOT) lists the specified files.
...
% of FG_list_allfiles_in_subdirs output.
...
% *Examples*
%
%   D = FG_list_allfiles_in_subdirs('*.m');
%     for ii=1:length(D), disp(D(ii).name); end;
%
%   % to find all files in the current directory and sub directories
%   D = FG_list_allfiles_in_subdirs('**\*')
%
%   % If no output is specified then the files are sent to 
%   % the screen.
%   FG_list_allfiles_in_subdirs('c:\program files\windows *\*.exe');
%   FG_list_allfiles_in_subdirs('c:\program files\windows *\**\*.dll');
%
%   % Using the test function to find files modified today
%   FG_list_allfiles_in_subdirs('c:\win*\*','datenum>floor(now)');
%   % Using the test function to find files of a certain size
%   FG_list_allfiles_in_subdirs('c:\program files\win*\*.exe','bytes>1024 & bytes<1048576');
%   % Using the test function to find files modified in 2011
%   FG_list_allfiles_in_subdirs('c:\win*\*','strfind(date, ''2011'')');
%
%   % Using the 3rd input to shorten output name
%   FG_list_allfiles_in_subdirs([matlabroot, '\*.txt'], '', 'C:\Program Files\')
%   % Using the 3rd input to shorten output name
%   FG_list_allfiles_in_subdirs([matlabroot, '\*.txt'], '', 1)
%
%
% See also DIR
%
...
elseif strcmp(wildpath,'**')
  ...
  D = FG_list_allfiles_in_subdirs([prepath postpath(2:end)]);
  ...
  Dt = dir(''); 
  for ii = 1:length(tmp),
    Dt = [Dt; FG_list_allfiles_in_subdirs([prepath tmp(ii).name filesep wildpath postpath])]; %#ok<AGROW>
  end
  D = [D; Dt];
  ...
  if ~isdir(prepath)
    % Fix case like FG_list_allfiles_in_subdirs('path*\...') where prepath is not a directoty
    % name.
    prepath = '';
  end
  ...
  D = dir(''); 
  for ii = 1:length(tmp),
    D = [D; FG_list_allfiles_in_subdirs([prepath tmp(ii).name postpath])]; %#ok<AGROW>
  end
...
if (nargin>=2 && ~isempty(varargin{1})),
  try
    test_tf = evaluate(D, varargin{1});   % cliff, function: "evaluate ", seems so good
