function FG_advanced_MVPA_easy_mode(imgs,mask,regressors,selectors)
...
if nargin==0   
   ...
   mask =  spm_select(1,'.img|.nii','Select a whole brain mask', [],pwd,'^wholebrain.*');
...
% wholevol = ones(64,64,40); %% use a self-defined whole-image mask, this is usually too big to fail
% subj = initset_object(subj,'mask','wholebrain_mask',wholevol);
...
try   %%% try to run wiht AFNI fisrt
    %%%% 1. convolving with the haemodynamic response fucntion
    %%%  it requires AFNI and the ''waver'' function to work. If you don't have access to AFNI, 
    %%%  then you'll need to use another program to convolve the regressors, save them out as text files, 
    %%%  load them in to Matlab normally, and then use INITSET_OBJECT.M to put them into a regressors object
    ...
    subj = convolve_regressors_afni(subj,'conds','runs', ...
                            ...
                            'binarize_thresh',0.5, ...
                            'do_plot',true); 
catch me  %% if there is no AFNI, use the shift istead
    me.message
    %%%% 2. the haemodynamic response should peak around 5 seconds after
    %%%% stimulus onset, so for the TR=2s exps, shift 3 timepoints along(3*2=6)
    subj = shift_regressors(subj,'conds','runs',3);
    figure;
    conds=get_mat(subj,'regressors','conds');
    conds_shifted3=get_mat(subj,'regressors','conds_sh3');
    subplot(2,1,1);imagesc(conds(:,1:150))
    subplot(2,1,2);imagesc(conds_shifted3(:,1:150)) 
    %%% or just use the additional parameters to show these figures:  
    %%%%           subj = shift_regressors(subj,'conds','runs',3,'do_plot',true);

end

% After the convolving or shifting
% The first few timepoints are all-zeros, indicating that no conditions were active in the design, 
% and we may want to exclude these from the rest of our current analysis
subj = create_norest_sel(subj,'conds_sh3');
conds_sh3_norest=get_mat(subj,'selector','conds_sh3_norest');
...
subj = create_xvalid_indices(subj,'runs', ... 
               'actives_selname','conds_sh3_norest', ...
...
%% write the files back to the hard disk
%         % for afni users
%         write_to_afni(subj,'pattern','searchlight_results','run_1_detrend_shft+orig')
%         % for SPM users (searchlight_results.nii will be created)
%         write_to_spm(subj,'pattern','searchlight_results','fextension','.nii','new_header',true)
