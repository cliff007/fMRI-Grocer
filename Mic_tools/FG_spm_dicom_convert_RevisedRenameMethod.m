function out = FG_spm_dicom_convert_RevisedRenameMethod(hdr,opts,root_dir,format,output_pths)
...
% cliff: add the fifth input variable to specific special output paths
...
% cliff
if nargin==5
    if size(hdr,2)==size(output_pths,1)
        output_pths;
    elseif size(output_pths,1)==1
        output_pths=repmat(output_pths,[size(hdr,2),1]);
    else
        fprintf('\n===The number of output paths should be equal to the input images or just one===\n')
        return
    end
elseif nargin<5
    output_pths=repmat(pwd,[size(hdr,2),1]);
end
...
if (strcmp(opts,'all') || strcmp(opts,'mosaic')) && ~isempty(mosaic),
    fmos = convert_mosaic(mosaic,root_dir,format,output_pths);
end;
if (strcmp(opts,'all') || strcmp(opts,'standard')) && ~isempty(standard),
    fstd = convert_standard(standard,root_dir,format,output_pths);
end;
if (strcmp(opts,'all') || strcmp(opts,'spect')) && ~isempty(spect),
    fspe = convert_spectroscopy(spect,root_dir,format,output_pths);
end;
...
%_______________________________________________________________________
function fnames = convert_mosaic(hdr,root_dir,format,output_pths)
...
for i=1:length(hdr),
    ...
    % Output filename
    %-------------------------------------------------------------------
    fnames{i} = getfilelocation(hdr{i},root_dir,'Mosaic_',format,output_pths,i);  % cliff: : replace [ f ] to [ f_ ]
...
%_______________________________________________________________________
function fnames = convert_standard(hdr,root_dir,format,output_pths)
hdr = sort_into_volumes(hdr);
fnames = cell(length(hdr),1);
for i=1:length(hdr),
    fnames{i} = write_volume(hdr{i},root_dir,format,output_pths,i);
end;
return;
%_______________________________________________________________________
...
%_______________________________________________________________________
function fname = write_volume(hdr,root_dir,format,output_pths,i)

% Output filename
%-------------------------------------------------------------------
fname = getfilelocation(hdr{1}, root_dir,'standard_',format,output_pths,i);
...
%_______________________________________________________________________
function fnames = convert_spectroscopy(hdr,root_dir,format,output_pths)
fnames = cell(length(hdr),1);
for i=1:length(hdr),
    fnames{i} = write_spectroscopy_volume(hdr(i),root_dir,format,output_pths,i);
end;
return;
%_______________________________________________________________________

%_______________________________________________________________________
function fname = write_spectroscopy_volume(hdr,root_dir,format,output_pths,i)
% Output filename
%-------------------------------------------------------------------
fname = getfilelocation(hdr{1}, root_dir,'Spectro_',format,output_pths,i);
...
function fname = getfilelocation(hdr,root_dir,prefix,format,output_pths,i)

if nargin < 3
    prefix = 'f_'; % cliff: : replace [ f ] to [ f_ ]
end;
...
if strcmp(root_dir, 'flat')
    ...
    if checkfields(hdr,'SeriesNumber','AcquisitionNumber')
        if checkfields(hdr,'EchoNumbers')
            fname = sprintf('%s%s_%.4d_%.5d_%.6d_%.2d.%s', prefix, strip_unwanted(hdr.ProtocolName),...
                hdr.SeriesNumber, hdr.AcquisitionNumber, hdr.InstanceNumber,...
                hdr.EchoNumbers, format);  %% cliff: replace [strip_unwanted(hdr.PatientID)] to [strip_unwanted(hdr.ProtocolName)]
                                     %% cliff: replace [-%] to [_%]
        else
            fname = sprintf('%s%s_%.4d_%.5d_%.6d.%s', prefix, strip_unwanted(hdr.ProtocolName),...
                hdr.SeriesNumber, hdr.AcquisitionNumber, ...
                hdr.InstanceNumber, format);
        end;
    else
        fname = sprintf('%s%s_%.6d.%s',prefix, ...
            strip_unwanted(hdr.ProtocolName),hdr.InstanceNumber, format);
    end;

    % cliff
%     fname = fullfile(pwd,fname);
      fname = fullfile(deblank(output_pths(i,:)),fname);
    return;
end;
...
fname = sprintf('%s%s_%.5d_%.6d_%.2d.%s', prefix, protname,...
                hdr.AcquisitionNumber, hdr.InstanceNumber,...
                hdr.EchoNumbers, format);  %% cliff: replace [strip_unwanted(hdr.PatientID)] to [strip_unwanted(hdr.ProtocolName)]
                                     %% cliff: replace [-%] to [_%]
