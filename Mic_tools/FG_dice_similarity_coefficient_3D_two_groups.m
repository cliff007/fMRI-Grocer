function FG_dice_similarity_coefficient_3D_two_groups(order)
...
elseif strcmp(h_files,opts.files.oper{1})
  ...
  for i=1:size(subj1,1)
      ...
      elseif strcmp(h_files,opts.files.oper{2}) || strcmp(h_files,opts.files.oper{1})
        ...
        if size(subj1,1) ~=size(subj2,1)
            fprintf(['\nERROR:  The file number under Subject -' num2str(i) ' should be the same!\n\n'])
            return
        end
        ...
        overlay_dir=fullfile(out_dir,[tem1(1,[1:5, end-4:end]) '_overlay_' tem2(1,[1:5, end-4:end])]);  % cliff: what if the length of tem1 is less than 5??
        mkdir (overlay_dir);
