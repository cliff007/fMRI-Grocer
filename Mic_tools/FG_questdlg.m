function choise=FG_questdlg(dlgOptions, dlgTitle, defOption, qStr, bttnsOredring)
%% FG_questdlg
% Create and open a button dialog box with many buttons.
%
%% Syntax
%  FG_questdlg(dlgOptions); 
%  FG_questdlg(dlgOptions, dlgTitle);
%  FG_questdlg(dlgOptions, dlgTitle, defOption); 
%  FG_questdlg(dlgOptions,dlgTitle, defOption, qStr); 
%  FG_questdlg(dlgOptions, dlgTitle, defOption, qStr, bttnsOredring);
%
%% Description
...
%  function wasn't exited properly, figure should be closed via inline delete(figH)
...
% iSel=FG_questdlg(inputOptions, 'Demonstarte FG_questdlg', defSelection,...
...
%% Default params
if nargout>1
  error('MATLAB:FG_questdlg:WrongNumberOutputs',...
     'Wrong number of output arguments for QUESTDLG');
end
if nargin<1
  error('MATLAB:FG_questdlg:WrongNumberInputs',...
     'Wrong number of input arguments for FG_questdlg');
end
if nargin==1
   dlgTitle=' ';
end
...
if nargin>5
  error('MATLAB:FG_questdlg:TooManyInputs', 'Too many input arguments');
end
