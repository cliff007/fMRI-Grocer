function [h_files,fun_imgs,file_filter]=FG_module_select_files_undersubjects(groups,opts,file_filter_for_auto)
...
h_files=questdlg(opts.files.prom,opts.files.title,opts.files.oper{1},opts.files.oper{2},opts.files.oper{3},opts.files.oper{1}) ;
        ...
        if strcmp(h_files,opts.files.oper{1})   
            ...
            fun_imgs =  spm_select(inf,'any','Select all the fun_imgs of one subject', [],pwd,file_filter_for_auto);
            if FG_check_ifempty_return(fun_imgs) ,fun_imgs='return';  return; end
            fun_imgs=spm_str_manip(fun_imgs,'dt');
            file_filter=file_filter_for_auto;
        elseif strcmp(h_files,opts.files.oper{2})
            for g=1:size(groups,1)
                fun_imgs{g}=spm_str_manip(spm_select(inf,'any',['Select the fun_imgs of one subject:' groups(g,:)], [],pwd,file_filter_for_auto),'dt');
                if FG_check_ifempty_return(fun_imgs{g}), fun_imgs='return'; return; end
            end
            file_filter=file_filter_for_auto;
