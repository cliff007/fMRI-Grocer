function P = FG_spm_realign_asl(P,flags)
%%%%%%%% Revised version: add one more field (ctrfirst) to the "flags" variable
%%%%%%%% this field is used to lable the control or label image of ASL data
...
if isfield(flags,'ctrfirst') % add one more field (ctrfirst) to the "flags" variable
    ctrfirst=flags.ctrfirst; % this field is used to lable the control or label image of ASL data
else 
    ctrfirst=0; 
end     
...
% 	if nargout==0, save_parameters(P{1}); end;
    ...
    P{1}=cleanandsave_parameters(P{1},ref);  % revised "cleanandsave_parameters" rather than the original "save_parameters"
else
	Ptmp = P{1}(1);
	for s=2:numel(P),
		Ptmp = [Ptmp ; P{s}(1)];
...
% 		if nargout==0, save_parameters(P{s}); end;
        ...
        ref(2:2:end)=1;
        if(ctrfirst) ref=-ref; end;
        P{s}=cleanandsave_parameters(P{s},ref);   % change the SPM-defaulted P{s} in the revised 'cleanandsave_parameters' subfunction
	end;                                      % so the images after this function is different from the ones after 'spm_realign.m'
                                                  % this is specially for ASL data.
end;
     
if nargout==0, %% cliff, that means if you don't won't to change the original image header, you shoud have an output of this funciton
	% Save Realignment Parameters---------cliff: into file header
	%---------------------------------------------------------------------------
	for s=1:numel(P),
		for i=1:numel(P{s}),
			spm_get_space([P{s}(i).fname ',' num2str(P{s}(i).n)], P{s}(i).mat);  % cliff: set the new .mat values in the header, subfunction "create" in the 'spm_get_space' do this job
		end;
	end;
end;
...
if numel(P) > 2,
    ...
    Alpha = [A0 b];
    Alpha = Alpha'*Alpha;  % spm_atranspa is out, use A'*A
	det0  = det(Alpha);
	det1  = det0;
    ...
    spm_plot_convergence('Clear');  % spm_chi2_plot is out
end;
%-----------------------------------------------------------------------
...
%_______________________________________________________________________
function Vo=cleanandsave_parameters(V,ref)  % compare to the original function: Vo=cleanandsave_parameters(V)
...
for j=1:6
    refval=clQ(:,j);
    clQ(:,j)=refval-ref/(ref'*ref)*ref'*refval;    %% cliff: regress out ref (label-control) variable from 6 motion-papras with , 
    clQ(:,j)=clQ(:,j)-clQ(1,j);                     
end                                                
for j=1:n,
    nmat = spm_matrix(clQ(j,:));
    Vo(j).mat=nmat*V(1).mat;   % change the original Vo.mat with self-defined clQ variable
end
Q=cat(2,Q,clQ);  % the first 6 columns are the original motion-pars (original function), the second 6 columns are the orthogonalized motion-paras
save(fname,'Q','-ascii');
return;
%_______________________________________________________________________
