function any_retrun=FG_modules_enhanced_selection(def_prom_root,def_prom_groups,def_folderfilter,def_filefilter, varargin)
       ...
       %     if FG_check_ifempty_return(root_dir),  return; end
        ...
        case 'fi' % file
            if isempty(def_filefilter) 
                [h_files,fun_imgs,all_fun_imgs,file_filter]=FG_module_enhanced_select_files_undersubjects(groups,opts,'^sr.*img$|^sr.*nii$');
            else
                [h_files,fun_imgs,all_fun_imgs,file_filter]=FG_module_enhanced_select_files_undersubjects(groups,opts,def_filefilter);
            end
            if any(strcmpi('return',{h_files,fun_imgs,all_fun_imgs,file_filter})), any_retrun=any_retrun+1; return; end
         %   if FG_check_ifempty_return(fun_imgs),  return; end           
            assignin('caller','h_files', h_files) ;
            assignin('caller','fun_imgs', fun_imgs);
            assignin('caller','all_fun_imgs', all_fun_imgs);
