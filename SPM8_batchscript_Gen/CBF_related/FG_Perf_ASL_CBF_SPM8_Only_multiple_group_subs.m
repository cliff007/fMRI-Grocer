...
function FG_Perf_ASL_CBF_SPM8_Only_multiple_group_subs(SelfmaskedorNo)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        fprintf('\n');
        disp( '    -----------------------------------------------------------------------------');
        disp( '        Welcome to Use ASL-Perfusion Reconstruction Script(SPM2/5/8 Comparable)');
        disp(['                    Originally created by By H.Y. Rao et al,']);
        disp(['               Revised and improved by Senhua Zhu @CFN, UPenn Med.']);
        fprintf('            -------------------------------------------------------\n');
        disp(['                  http://www.fil.ion.ucl.ac.uk/spm/ext/#Grocer']);
        disp(['                       Current SPM Version is ',spm('Ver'),' ^_^']);
        fprintf('                    ------------------------------------\n');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        if nargin==0 
           SelfmaskedorNo='CBF';  % default is no selfmask  
        end
        
        %%%%%%%%%%%%%%%% parameters setup is done %%%%%%%%%%%%%%%%%%%   
           % file selection
                anyreturn=FG_modules_selection('','','','^sr.*img$|^sr.*nii$','r','g','fo','fi');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% start to loop subjects within groups %%%%%%%%%%%%%%%%%%%   
for g_num=1:size(groups,1)
        
        % assigning the subfolders of groups
        dirs=FG_module_assign_dirs(root_dir,dirs_tem,groups,g_num,folder_filter,h_folder,opts);         
        % assigning the self-masks of groups
        if strcmp(SelfmaskedorNo,'CBF-selfmasked')  % cliff add self-mask
            mask_imgs=FG_module_assign_mask(mask_imgs_tem,g_num,h_mask,opts);        
            mask_mats=spm_vol(mask_imgs);
            mask_Vs=spm_read_vols(mask_mats);     
        end
        % assigning is done 
        

    for dir_num=1:size(dirs,1)
          
        if strcmp(h_files,opts.files.oper{1})
            for img_num=1:size(fun_imgs,1) 
               Filename(img_num,:)=[fullfile(root_dir,deblank(groups(g_num,:))), filesep,deblank(dirs(dir_num,:)),filesep,deblank(fun_imgs(img_num,:))];
            end
        elseif strcmp(h_files,opts.files.oper{2})  
            for img_num=1:size(fun_imgs{g_num},1)
               Filename(img_num,:)=[fullfile(root_dir,deblank(groups(g_num,:))), filesep,deblank(dirs(dir_num,:)),filesep,deblank(fun_imgs{g_num}(img_num,:))]; 
            end  
        elseif strcmp(h_files,opts.files.oper{3})              
            fun_imgs=spm_select('FPList',[fullfile(root_dir,deblank(groups(g_num,:))), filesep,deblank(dirs(dir_num,:))],file_filter);
            Filename=fun_imgs; 
        end
        ...
        if ASLType == 0;  % PASL
              if g_num==1 && dir_num==1 && mod(size(Filename,1),2)==0
...
% cliff
   ...
   %            VB(k)  = spm_create_vol(VB(k));
    ...
    if strcmp(SelfmaskedorNo,'CBF-selfmasked')  % cliff add self-mask
        Mask=double(logical(mask_Vs(:,:,:,dir_num)));% cliff add
...
% cliff
   ...
   % save globalCBF globalCBF %EDIT(H)
      ...
      if CBFFlag ==1, 
         ...
         fprintf('\n\nResults Summary of: Group %d Subj %d:   %s  ...\n',g_num,dir_num,pth)
 ...
 %%%% an extra enhanced MeanCBF denoising procedure 
     movefil=spm_select('FPList', [root_dir,deblank(groups(g_num,:)), filesep,deblank(dirs(dir_num,:))], ['^rp_\w*\.txt$']);
     if ~isempty(movefil) && size(movefil,1)==1 
         ...
         CBF_files=spm_select('FPList', [root_dir,deblank(groups(g_num,:)), filesep,deblank(dirs(dir_num,:))], '^CBF.*.img$|^CBF.*.nii$');
         % run outlier cleaning to get a outlier-reduced meanCBF img
         FG_outlier_clean_after_CBF(CBF_files,[root_dir,deblank(groups(g_num,:)), filesep,deblank(dirs(dir_num,:))]);
         %%%% extra procedure is done
     else
 ...
 fprintf('\n\n\n-----This run of CBF-calculations is done---------\n')
